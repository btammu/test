...
curl -X POST 'https://budda-tammu.atlassian.net/rest/api/latest/issue' --header 'Accept:application/json' --header "Authorization: Basic $newtoken" -d '{"fields": {"project":{"key":"BTAM"}, "summary": "REST EXAMPLE API", "description": "Creating an issue via REST API", "issuetype": {"name": "Task"}}}'
